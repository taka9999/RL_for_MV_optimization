...
Regime-switching portfolio control is challenging because latent-state
inference, terminal credit assignment, and transaction costs interact over
long horizons. The framework therefore restricts the learned policy to
economically interpretable, center-relative geometry and estimates only
lower-dimensional coefficients or boundaries, rather than learning an
unrestricted state-to-portfolio map. For the costly problem, the frictionless solution provides a structured initialization so that Stage~2 learns a correction around an economically meaningful benchmark rather than the full policy from scratch.
...
\label{sec:actuarial}
\paragraph{Actuarial relevance and potential applications.}
Life insurers and pension funds manage long-duration liabilities through
investment strategies whose consequences accumulate over many rebalancing
dates. Frequent adjustment can be impractical because of proportional
transaction costs, liquidity constraints, capital limits, and governance
requirements. Moreover, persistent market regimes affect not only expected
returns but also volatility, correlation, credit risk, and the relative
attractiveness of return-seeking and defensive assets. Partial information
and trading frictions are therefore central features of the institutional
investment problem rather than secondary implementation details.

Within the present framework, the terminal target \(z\) can be interpreted
as a reduced-form accumulation, reserve, or funding objective, including objectives linked to credited-rate
commitments. The posterior belief \(p_t\) provides a probabilistic summary of the
prevailing market environment, the Stage~1 policy specifies a
regime-aware frictionless risk exposure, and the Stage~2 no-trade region
translates the trade-off between tracking that exposure and limiting costly
turnover into a transparent rebalancing rule that can support governance
oversight. In a fixed-income or
liability-driven-investment application, the same architecture could support
regime-aware allocation between return-seeking credit and defensive
government-bond sleeves, or govern the rebalancing of duration and credit
overlays. These examples are prospective applications of the architecture;
they are not evaluated in the numerical experiments reported here.

The model is not a complete asset--liability management system. It does not
include stochastic liability cash flows, interest-rate-sensitive liability
valuation, solvency capital, accounting constraints, taxes, cash-flow
matching, or endogenous market impact. A broader ALM formulation could
augment the state with liability values, funding ratios, and yield-curve
factors; replace the terminal-wealth criterion with a surplus- or
liability-relative objective; and impose institution-specific capital,
liquidity, and asset-allocation constraints. The present contribution should
therefore be viewed as a regime-aware portfolio-control building block for
such a system.
...
The implementation fixes the wealth dependence through $(x-\omega)$ and
learns only low-dimensional belief- and time-dependent coefficients. A representative actor is
\begin{equation}
...
The pure martingale-difference update is closest to the continuous-time
theory. The implemented mixed signal sets $\eta=0.3$, assigning 30\% weight
to the normalized full-horizon tail residual. This is an algorithmic
finite-sample refinement rather than a new control theorem.
...
\subsection{Direct Boundary policy optimization for Stage~2}

The QVI is the natural theoretical benchmark for the transaction-cost
problem. Under the optimization scheme considered here, joint learning of
the QVI residual and intervention map did not converge reliably. Stage~2
therefore evaluates Direct Boundary policy optimization, which directly
optimizes a restricted no-trade policy through simulated wealth paths. The main sources of numerical difficulty were:
\begin{itemize}
...
\end{itemize}
Direct Boundary policy optimization uses pathwise gradient descent on the terminal objective, without a separate learned transaction-cost critic or value function.

The distinction between the theoretical benchmark and the reported objective is important. The QVI above retains the Stage~1 mean--variance terminal condition, whereas the implemented Direct Boundary policy is trained using the terminal utility and regularization objective in \eqref{eq:band_loss}. Stage~2 is therefore not claimed to solve the exact transaction-cost extension of the Stage~1 problem. The frozen Stage~1 policy is used as a state-dependent structural feature and structured initialization, and all reported Stage~2 policy comparisons are interpreted under the separate transaction-cost-aware objective.
...
Typical terminal utilities include logarithmic, square-root, and constant-relative-risk-aversion (CRRA) utility, all of which require an explicit treatment of nonpositive wealth.

Direct Boundary policy optimization is related to the HJB-guided structured-initialization strategy of Mulvey et al.~\cite{mulvey2020}: a tractable frictionless solution supplies information that makes the transaction-cost problem easier to learn. The QVI derivation above explains why a no-trade region and center-relative coordinates are natural, but the reported optimizer directly trains the restricted boundary policy class because the attempted QVI critic/intervention actor was unstable. The QVI remains the theoretical benchmark, but the evaluated boundary policy is not its numerical solution.
...
\item \textbf{Cumulative transaction cost} is defined as (gross terminal wealth $-$ net terminal wealth), i.e.\ by construction from the two quantities above rather than as an independently accumulated series. This identity is an accounting definition rather than a counterfactual no-cost re-simulation.
...
Evaluation scenario set & Same fixed set of 1000 held-out scenarios for all five training seeds (evaluation seed 999). \\
...
Checkpoint policy & Selected by validation utility every 25 iterations on 64 held-out validation paths. \\
Gross/net convention & Cumulative transaction cost is defined as gross terminal wealth minus net terminal wealth. \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{Repeated-seed robustness protocol}
...
the Student-$t$ critical value with four degrees of freedom. Outputs from deterministic benchmark policies are identical
across training seeds, providing
an additional consistency check on scenario alignment.

The regime-dependent-covariance experiment in
Section~\ref{sec:regcov_results} was not part of the five-seed replication
and remains a single-seed exploratory result.
...
\centering
\small
\caption{Stage~2 performance across five training seeds, each evaluated on
the same $1000$ held-out paths. Entries are means of the five seed-level
statistics; brackets report 95\% Student-$t$ intervals across training seeds
for net terminal wealth. Turnover and band width are averaged over the
$252$ decision dates. Gross terminal wealth follows the accounting definition
in Section~\ref{sec:experimental_design}; ``--'' denotes a metric that is not
applicable to the comparator.}
...
\caption{Stage~2 cost and turnover comparison across methods: cumulative transaction cost, turnover, trade count, and band width. Bars show the five-seed mean; error bars show the 95\% training-seed interval. Band width is defined only for Center+DNNBand because the comparator
policies do not contain a no-trade region. The ``N/A'' labels denote
non-applicability, not zero-width bands.}
...
The results illustrate how an institutional investment rule can separate three economically distinct tasks: estimating the market environment, selecting a frictionless risk exposure, and deciding whether the benefit of rebalancing justifies its cost. The belief state is not merely a forecasting output; it enters the actor, critic, and transaction-cost policy as a decision variable. The band component provides an interpretable rebalancing rule:
absent a leverage-cap violation, trading is deferred until the current
position departs sufficiently from the belief- and horizon-dependent center.
The evaluated implementation combines this band rule with the common
leverage-feasibility projection described in Section~7. Consequently, a trade
may be triggered either by a band crossing or by a leverage-cap violation.
...
Across the five-seed replication, the evaluated band-plus-cap policy reduced the mean number of trades by approximately 30\% relative to daily center rebalancing (Section~\ref{sec:stage2_vs_stage1}), which can be read as evidence that the framework offers an operational turnover-discipline benefit in addition to its net-performance statement, not only as a net-wealth result. These results are methodological demonstrations rather than investment recommendations: actual market impact, transaction-cost nonlinearity, and liability dynamics are not modeled here, and the reported experiments do not constitute a complete asset--liability management evaluation.
...
The SJM produced a practically usable persistent segmentation in the reported experiment. The analysis does not establish superiority over HMM-based estimation. A controlled comparison should examine label stability, parameter error, posterior filtering accuracy, and downstream portfolio performance under common data and evaluation paths. The evidence supports only the operational usefulness of combining persistent offline segmentation with online Bayesian belief updating.
...
The Stage~2 results have a similarly specific interpretation. Daily rebalancing to the frictionless center has lower mean net wealth than monthly rebalancing, consistent with repeated proportional costs offsetting the marginal tracking benefit. Center+DNNBand has the highest mean net wealth among the reported comparators and reduces mean trade count relative to daily rebalancing. Separate summaries of turnover, cumulative transaction cost, trade count,
and band width provide a more direct view of implementation effects than
net terminal wealth alone (Table~\ref{tab:stage2_means}, Figure~\ref{fig:stage2_cost_turnover}). The available comparisons do not determine whether the net-wealth difference is driven primarily by trading frequency, boundary asymmetry, belief conditioning, or time-to-go conditioning; separating these mechanisms causally requires an ablation analysis that has not been performed.
...
\min\{-V_t-\mathcal LV,\ \mathcal MV-V\}=0,
\end{equation}
where $\mathcal M$ accounts for proportional wealth loss and a post-trade target map. Future work should focus on stabilizing numerical learning of this QVI: continuation/intervention sampling, obstacle-compatible losses, target networks for $\mathcal M V$, curriculum learning from a Direct Boundary policy, and smooth trigger approximations are natural candidates.
...
The implementation, experiment configurations, and scripts used to produce
the tables and figures are available from the author upon reasonable request.
All numerical experiments use synthetic data generated from the models
specified in the paper. Reproducibility of the exploratory
regime-dependent-covariance experiment is limited by the incomplete
