...
\end{itemize}
In trial runs, the QVI residuals and intervention trigger did not converge reliably and produced unstable actor updates. The final experiment therefore uses \emph{Direct Boundary policy optimization}: a restricted no-trade policy is optimized directly through simulated trajectories by pathwise gradient descent on the terminal objective, without a separate learned transaction-cost critic or value function.
...
The training and evaluation code uses this same pre-/post-trade timing convention throughout; the recursion version, checkpoint, and evaluation seeds are recorded in the archived manifest for every training seed (Table~\ref{tab:stage2_config}).
...
In the reported configuration, $U$ is logarithmic utility, $U(x)=\log\bigl(\max(x,\epsilon)\bigr)$ with positivity floor $\epsilon=10^{-10}$ applied directly inside the utility evaluation (not as a separate bankruptcy-penalty term); no additional nonpositive-wealth penalty is added to the loss. The training code exposes two separate configuration fields that could plausibly be mistaken for a risk-aversion parameter, and neither functions as one for the reported configuration. First, \texttt{utility\_gamma} (recorded as $2.0$) is passed as the exponent argument to the utility function; it is used only by the ``power'' (CRRA) branch, where it would set the CRRA risk-aversion coefficient, but the logarithmic-utility branch actually selected by \texttt{utility\_kind=log} ignores this argument entirely, so $U(x)=\log(\max(x,\epsilon))$ regardless of its value. Second, \texttt{gamma\_risk} (recorded as $5.0$) enters only an alternative QVI-heuristic gap-width formula active under \texttt{gap\_mode=qvi\_exp}; the reported configuration (\texttt{gap\_mode=direct}) never invokes that formula, so \texttt{gamma\_risk} has no effect on the trained policy, the loss, or the boundary gaps. Both fields are retained in the archived configuration only because they are general-purpose fields of the training script; neither is, for this configuration, a risk-aversion parameter multiplying the utility. The loss coefficients used are $\lambda_{to}=5\times10^{-4}$ (\texttt{turnover\_coef}), $\lambda_g=10^{-5}$ (\texttt{gap\_l2\_coef}), and $\lambda_{gl}=10^{-5}$ (\texttt{gross\_lev\_coef}), as recorded in Table~\ref{tab:stage2_config} and the archived manifest for every training seed. The checkpoint used for evaluation (\texttt{best\_checkpoint.pt}) is selected, every $25$ training iterations, as the iteration with the highest mean validation utility $\E[U(x_T)]$ over $64$ held-out validation paths generated independently of both the training and the $1000$-path evaluation set. This validation utility is computed by the same band-only simulator used in training, i.e.\ without the evaluation-time leverage cap \eqref{eq:eval_projection}: the checkpoint-selection criterion and the final reported evaluation metrics are therefore not evaluated under an identical policy, since only the latter additionally applies the hard leverage cap on top of the learned band.
...
\centering
\small
\caption{Selected Stage~1 configuration parameters, read from the archived \texttt{run\_config.json} for the common-covariance track and cross-checked against the training-script argument defaults in \texttt{poemv\_rs/train.py} and \texttt{poemv\_rs/agent.py}. Optimizer and actor-mixing hyperparameters (rows below the double rule) are set by the same training-script CLI and were confirmed identical across the two tracks in every archived configuration file inspected; they are not independently re-tuned per track.}
\label{tab:configs}
...
Episodes per outer iteration & $16$ & varies by archived run; not fixed to a single value for this exploratory track \\
...
Critic polynomial degree ($m$, $m_g$) & $2$, $2$ (training-script default, not CLI-configurable) & same \\
Gradient clipping norm & $1.0$ (training-script default) & same \\
...
\centering
\small
\caption{Stage~2 (Center+DNNBand) configuration for the five-seed replication. All entries are read from the archived \texttt{run\_config.json}, \texttt{eval\_config.json}, \texttt{aggregation\_config.json}, and \texttt{run\_manifest.csv} for each training seed, not transcribed from memory.}
...
Training/evaluation code provenance & Version-controlled commit identifier embedded in every checkpoint; per-seed evaluation code version and checkpoint/manifest hashes are given in Appendix~\ref{app:availability} \\
\bottomrule
\end{tabular}
\end{table}
...
The regime-dependent-covariance experiment was not included in the five-seed replication. Its table below reports a separate single-seed exploratory run, retained only to demonstrate implementability of the approximate filtering-and-control layer; no wealth-distribution figure is available for this single-seed run, so only the summary table and discussion are reported.
...
Per-iteration Stage~2 training diagnostics (terminal-objective and boundary-loss learning curves) are not shown as a figure here, since no such multi-seed visualization exists among the archived outputs; the underlying per-iteration training metrics remain available in the archived run outputs for each of the five training seeds.
...
\item A true, time-resolved cap-saturation rate has not been measured. The reported \texttt{share\_\allowbreak paths\_\allowbreak avg\_\allowbreak leverage\_\allowbreak near\_\allowbreak cap} diagnostic (threshold: whole-path average gross leverage $\ge 1.980$) is a path-average proxy over the fraction of evaluation paths, not a per-timestep saturation rate, and should not be read as one. This diagnostic is uninformative for Stage~2: because every reported Stage~2 comparator is rescaled to gross leverage at most $1.5$ by the evaluation-time cap \eqref{eq:eval_projection}, its whole-path average gross leverage can never reach the $1.980$ threshold, so the statistic is zero by construction for all four Stage~2 methods. It is meaningful only for the uncapped Stage~1 policy, where the archived values are approximately $1$--$1.5\%$ of evaluation paths across the five training seeds. The specific numeric value of the $1.980$ threshold is a fixed constant in the aggregation script with no derivation recorded in the code or configuration files linking it to the Stage~1 action bound or the Stage~2 leverage cap; its precise provenance is therefore not established and is not further interpreted here.
...
+\lambda_{co}\E[(R_{\mathrm{cont}}R_{\mathrm{obs}})^2],
\end{equation}
where $r^-=\min(r,0)$. This was the intended critic-based Stage~2 direction. In practice, simultaneous fitting of these residuals and the intervention actor was unstable, motivating the Direct Boundary policy optimization implementation reported in the main text.
...
All Stage~1 and Stage~2 training and evaluation code, the aggregation script used to compute every five-seed table and figure in Sections~\ref{sec:stage1_results}--\ref{sec:stage2_vs_stage1} from archived per-seed evaluation output, and the archived per-seed configuration, checkpoint, and evaluation manifests are maintained in a version-controlled repository. Each of the five training seeds' checkpoints embeds the exact source-code commit identifier used to produce it; a subsequent evaluation-only commit, which did not alter any trained model, was used to correct an evaluation-invocation error affecting two seeds (Section~\ref{sec:stage1_results}). Per-seed checkpoint and manifest SHA-256 hashes are recorded in the archived \texttt{run\_manifest.csv}. The repository is not yet public; it will be made available upon request to the author or at a public repository URL to be added at publication, subject to institutional approval.

Every numerical result, table, and figure in this paper is computed by the aggregation script directly from archived per-seed evaluation CSV/parquet output, with no manually transcribed or hand-computed values other than simple arithmetic shown explicitly in the text.
