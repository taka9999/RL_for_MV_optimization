...
\vspace{0.45cm}
{\small\itshape Draft version, July 29, 2026. Five-seed common-covariance robustness analysis incorporated.\par}
\vfill
...
\begin{abstract}
This paper develops a structure-guided learning framework for multi-period portfolio optimization with latent market regimes and proportional transaction costs. Stage~1 studies a precommitted mean--variance problem. Under common observation covariance, Wonham filtering yields a belief-state diffusion, while the entropy-regularized HJB equation identifies a Gaussian exploratory policy and a wealth-quadratic value function. These results motivate a low-dimensional actor--critic rather than an unrestricted state-to-weight learner. A statistical jump model supplies regime-dependent parameter estimates to the online filter. For regime-dependent covariance, an exact Gaussian Bayes correction for the adopted discrete-time hidden Markov observation model is combined with a moment-matched control approximation and an approximate HJB structure.

Stage~2 freezes the frictionless policy as a state-dependent center for costly trading. A transaction-cost QVI provides the theoretical benchmark, but joint critic--intervention learning was numerically unstable; the reported implementation therefore learns asymmetric no-trade boundaries through direct pathwise optimization.

Across five independent training seeds evaluated on a common set of $1000$ held-out paths, the common-covariance Stage~1 policy was highly stable, with mean terminal wealth $1.1811$ and a 95\% Student-$t$ interval across training seeds of $[1.1808,1.1814]$. Center+DNNBand achieved mean net terminal wealth $1.2009$ with interval $[1.1807,1.2211]$, exceeded daily center rebalancing by $0.0910$ with interval $[0.0709,0.1112]$, and reduced mean trade count from $252$ to $175.8$. Its descriptive difference from Stage~1 was $0.0198$ with interval $[-0.0003,0.0400]$; because this interval includes zero and the two stages optimize different objectives, no Stage~1-versus-Stage~2 dominance claim is made. The regime-dependent-covariance experiment remains exploratory and single-seed. The framework is interpreted as a portfolio-control building block for institutional and actuarial applications rather than a complete asset--liability management model.
\end{abstract}
...
The common-covariance Stage~1 and Stage~2 experiments above are empirically validated on a common set of independently generated evaluation paths, repeated across five independent training seeds. For each training seed, the same fixed evaluation scenarios are used to compute seed-level summary statistics; these are then combined across seeds into a mean, a between-seed standard deviation, and a 95\% Student-$t$ interval with four degrees of freedom. Where two policies are compared on the same paths, paired differences are computed at the evaluation-episode level before aggregation across seeds. This design quantifies sensitivity to training initialization under a fixed market-evaluation design; it is not a claim about market-distribution uncertainty. Under this protocol, Stage~2 shows materially greater training-seed variability than Stage~1.
...
The archived configuration for the exploratory regime-dependent-covariance experiment (Section~\ref{sec:regcov_results}) records the estimation horizon and optimization settings, but it does not retain the exact rolling windows, feature-by-feature normalization statistics, or the random seed used to generate the estimation sample. This is a reproducibility limitation of that single-seed experiment specifically: unlike the common-covariance Stage~1/Stage~2 pipeline, whose per-seed configuration, checkpoints, and evaluation manifest are fully archived (Table~\ref{tab:stage2_config}), the regime-dependent-covariance run cannot currently be exactly reproduced from its saved artifacts alone, and it is excluded from the five-seed Phase~1A replication for this reason among others.
...
\end{align}
the outer $\E[\cdot]$ denotes the average over simulated episodes, $N=252$ is the number of daily steps in the one-year horizon, and the factor $\tfrac12$ in $\mathcal L_g$ is the average (not sum) of the squared lower- and upper-gap norms over the two risky assets. In the implemented code, $\lambda_U$ is the scalar \texttt{utility\_scale} and $s_U$ is an additive shift \texttt{utility\_shift}; for the Phase~1A runs $\lambda_U=1$ and $s_U=0$, so the term reduces to $-\E[U(x_T)]$ as in a standard terminal-utility objective. The physical proportional transaction cost is already deducted from wealth in \eqref{eq:tc_wealth}. The additional turnover term $\mathcal L_{to}$ is therefore a deliberate governance and training regularizer---and a simple proxy for unmodeled market impact---rather than a second accounting deduction of the same transaction cost. The gap term $\mathcal L_g$ discourages uninformatively wide bands; the gross-leverage term $\mathcal L_{gl}$ is the only leverage control active during training; the separate, evaluation-only hard cap is discussed above and in the Numerical Study (Section~\ref{sec:stage1_results} onward).

For Phase~1A, $U$ is logarithmic utility, $U(x)=\log\bigl(\max(x,\epsilon)\bigr)$ with positivity floor $\epsilon=10^{-10}$ applied directly inside the utility evaluation (not as a separate bankruptcy-penalty term); no additional nonpositive-wealth penalty is added to the loss. The training code exposes two separate configuration fields that could plausibly be mistaken for a risk-aversion parameter, and neither functions as one for the Phase~1A configuration. First, \texttt{utility\_gamma} (recorded as $2.0$) is passed as the exponent argument to the utility function; it is used only by the ``power'' (CRRA) branch, where it would set the CRRA risk-aversion coefficient, but the logarithmic-utility branch actually selected by \texttt{utility\_kind=log} ignores this argument entirely, so $U(x)=\log(\max(x,\epsilon))$ regardless of its value. Second, \texttt{gamma\_risk} (recorded as $5.0$) enters only an alternative QVI-heuristic gap-width formula active under \texttt{gap\_mode=qvi\_exp}; Phase~1A's configuration (\texttt{gap\_mode=direct}) never invokes that formula, so \texttt{gamma\_risk} has no effect on the trained policy, the loss, or the boundary gaps. Both fields are retained in the archived configuration only because they are general-purpose fields of the training script; neither is, for this configuration, a risk-aversion parameter multiplying the utility. The loss coefficients used are $\lambda_{to}=5\times10^{-4}$ (\texttt{turnover\_coef}), $\lambda_g=10^{-5}$ (\texttt{gap\_l2\_coef}), and $\lambda_{gl}=10^{-5}$ (\texttt{gross\_lev\_coef}), as recorded in Table~\ref{tab:stage2_config} and the archived manifest for every training seed. The checkpoint used for evaluation (\texttt{best\_checkpoint.pt}) is selected, every $25$ training iterations, as the iteration with the highest mean validation utility $\E[U(x_T)]$ over $64$ held-out validation paths generated independently of both the training and the $1000$-path evaluation set.

Typical terminal utilities include logarithmic, square-root, and CRRA utility, all of which require an explicit treatment of nonpositive wealth. An earlier, incompletely archived exploratory run did not record the exact utility, utility floor or bankruptcy penalty, or the coefficients $(\lambda_U,\lambda_{to},\lambda_g,\lambda_{gl})$, and is not part of the seed-aggregated inference reported starting in Section~\ref{sec:stage1_results}.
...
\centering
\small
\caption{Stage~2 (Center+DNNBand) configuration for the Phase~1A five-seed replication. All entries are read from the archived \texttt{run\_config.json}, \texttt{eval\_config.json}, \texttt{aggregation\_config.json}, and \texttt{run\_manifest.csv} for each training seed, not transcribed from memory.}
...
Training/evaluation code commit & \texttt{ba22728} (embedded in every checkpoint); Stage~1 evaluation for seeds $1$--$2$ was re-run under commit \texttt{04c4f2e} after a path-quoting invocation bug fix, without retraining \\
Manifest / checkpoint hashes & Recorded per seed in \texttt{run\_manifest.csv} (SHA-256) \\
\bottomrule
\end{tabular}
\end{table}
...
Where two methods were evaluated on the same simulated paths, paired differences were first calculated at the episode level, summarized within each training seed, and then aggregated across the five training seeds. The run manifest records the training seed, checkpoint path and hash, evaluation directory, configuration, evaluation-path count, and run status. Two empty Stage~1 evaluation directories produced by a previously identified path-quoting invocation error were excluded; the corrected evaluations used the original completed checkpoints and did not retrain the policies. No independent scenario-hash or per-path seed identifier was persisted alongside the stored evaluation output (only the integer path index, $0$--$999$); each path's underlying random scenario is instead determined deterministically by the fixed evaluation seed ($999$) and path index via the simulator's seeding convention, and was not independently re-verified path-by-path. As a consistency check on this convention having been applied identically across all five training seeds, the equal-weight (Stage~1) and equal-weight-monthly (Stage~2) comparators do not depend on the trained policy. Byte-identical deterministic benchmark outputs across all seeds provide a strong consistency check that the same evaluation scenarios were used.
...
\centering
\includegraphics[width=\linewidth]{file/phase1a/fig1_mean_vs_std.png}
...
\begin{subfigure}[t]{0.49\textwidth}
\centering
\includegraphics[width=\linewidth]{file/phase1a/fig5_training_seed_variability.png}
\caption{Mean terminal wealth by individual training seed, Stage~1 (left) and Stage~2 (right).}
\end{subfigure}
\caption{Training-seed robustness for the common-covariance HJB-guided policy (Stage~1, left panels) and Center+DNNBand (Stage~2, right panels). Both stages are shown together because the same five training seeds and evaluation design underlie both; Stage~1's near-total overlap of points contrasts with Stage~2's visibly larger spread.}
...
The regime-dependent-covariance experiment was not included in the five-seed Phase~1A replication. Its table below reports a separate single-seed exploratory run and is retained only to demonstrate implementability of the approximate filtering-and-control layer; the corresponding figure could not be regenerated from a retained image file and has been removed rather than shown as a placeholder (Section~\ref{sec:regcov_results} retains the table and discussion only).

\begin{table}[H]
\centering
\small
\caption{Single-seed exploratory regime-dependent-covariance held-out simulation summary. Not part of the five-seed Phase~1A replication; no training-seed interval is reported.}
...
Table~\ref{tab:stage2_means} reports the five-seed transaction-cost-aware comparison. An earlier incompletely archived single-seed illustration has been superseded by the fully traceable five-seed Phase~1A replication and is not used in the present inference; it is not mixed into the table below.
...
Center+DNNBand achieved mean net terminal wealth $1.2009$ (95\% training-seed interval $[1.1807,1.2211]$), mean gross terminal wealth $1.2196$, mean cumulative transaction cost $0.0187$, mean turnover $0.0397$, a mean of $175.8$ trades, and mean band width $1.740$. Its paired difference against daily center rebalancing was $0.0910$ with interval $[0.0709,0.1112]$, which excludes zero. Center+DNNBand reduced the mean number of trades from $252$ under daily center rebalancing to $175.8$, a reduction of approximately 30\%. This reduction in trade frequency accompanies, but is not shown to causally explain, the net-wealth improvement: separating the contribution of trade frequency from boundary asymmetry, belief conditioning, and time-to-go conditioning would require an ablation study that has not been performed.
...
\caption{Terminal wealth distributions pooled across five training seeds and $1000$ evaluation paths each. Left: Stage~1 (RL, equal weight, minimum variance). Right: Stage~2 (Center+DNNBand and the three comparators), net of transaction costs. Horizontal bars mark the pooled mean.}
...
The previously reported single-seed Stage~2 training diagnostics (terminal-objective and direct-boundary-loss learning curves) could not be regenerated from a retained image file for this revision and have been removed rather than shown as placeholders; the underlying per-iteration training metrics remain available in the archived run outputs for each of the five training seeds.
...
For regime-dependent covariance, the discrete-time Gaussian prediction--correction filter is exact for the specified hidden Markov observation model, but the belief-averaged effective diffusion and its HJB are moment-matched approximations. For transaction costs, the QVI is the theoretical benchmark and the center-relative critic/intervention actor is exploratory. Center+DNNBand, the reported Direct Boundary RL method, produced a numerically usable pathwise policy across all five training seeds, with a separate terminal objective; it is not a numerical solution of that QVI.
...
The common-covariance Stage~1 and Stage~2 experiments have now been verified across five independent training seeds on a common evaluation design (Section~\ref{sec:stage1_results}--\ref{sec:stage2_vs_stage1}), which resolves the single-seed limitation noted in earlier drafts for those two components specifically. The following limitations remain:
\begin{enumerate}[label=(\arabic*)]
...
\item Per-timestep wealth trajectories were not persisted by the evaluation code, so a time-resolved (rather than terminal-only) multi-seed wealth-path analysis is not currently available.
\item A true, time-resolved cap-saturation rate has not been measured. The reported \texttt{share\_\allowbreak paths\_\allowbreak avg\_\allowbreak leverage\_\allowbreak near\_\allowbreak cap} diagnostic (threshold: whole-path average gross leverage $\ge 1.980$) is a path-average proxy over the fraction of evaluation paths, not a per-timestep saturation rate, and should not be read as one.
\item Separate lower- and upper-boundary summary statistics were not persisted by the evaluation code; only the combined band width is reported.
...
The five-seed common-covariance study provides different robustness conclusions for the two learning stages. Stage~1 converged to nearly identical held-out performance across training seeds, with mean terminal wealth $1.1811$ and very small between-seed dispersion. In Stage~2, Center+DNNBand achieved the highest average net terminal wealth among the reported transaction-cost-aware comparators and retained a positive paired difference relative to daily and monthly center rebalancing. It also reduced mean trade count by approximately 30\% relative to daily rebalancing. However, Stage~2 was materially more sensitive to training initialization, and its descriptive net-wealth difference from Stage~1 had a 95\% interval that included zero. The evidence therefore supports robustness relative to the reported Stage~2 comparators, not a claim that Stage~2 statistically dominates the frictionless Stage~1 policy.

The regime-dependent-covariance evidence remains exploratory and single-seed (Section~\ref{sec:regcov_results}) and is not covered by the five-seed robustness statements above. Center+DNNBand is not a numerical solution of the transaction-cost QVI derived in this paper; it is a structurally motivated but separately optimized implementation. The paper's contribution is the selective use of financial structure -- filtering, HJB-consistent policy geometry, and a frictionless center -- to constrain what must be learned, not a claim that either stage is a verified optimal control solution. An actuarial application of this framework would require a further extension to a complete asset--liability management setting, incorporating stochastic liabilities, capital constraints, and institution-specific admissibility rules.
...
Table~\ref{tab:full_intervals} reports 95\% Student-$t$ training-seed intervals for evaluation metrics not already fully tabulated with intervals in the main text (Tables~\ref{tab:common_metrics} and \ref{tab:stage2_means} report only the terminal-wealth interval). Utility-consistent metrics are reported only for the policies explicitly evaluated under the terminal utility objective (Stage~1 RL and all four Stage~2 comparators); they were not computed for the static Stage~1 equal-weight and minimum-variance baselines, which are not part of the utility-based comparison. ``--'' denotes a metric not applicable to that method (e.g.\ band width for non-DNN-band Stage~2 comparators, or Stage~2-specific cost/turnover metrics for the Stage~1 policies, which do not trade with proportional cost in the Stage~1 evaluation).

\begin{table}[H]
\centering
\footnotesize
\caption{Distributional and utility-consistent metrics, 5-seed mean with 95\% Student-$t$ interval across training seeds (Stage~1: within-seed statistic over $1000$ paths; Stage~2: as Table~\ref{tab:stage2_means}). Target MSD and mean shortfall are derived from evaluation terminal wealth, not native solver outputs. Mean terminal utility and its log-utility certainty equivalent are reconstructed from archived episode-level terminal wealth using $U(x)=\log(\max(x,10^{-10}))$, the exact positivity floor used in training.}
...
\centering
\footnotesize
\caption{Stage~2 cost/turnover metrics, 5-seed mean with 95\% Student-$t$ interval across training seeds; each per-seed value is already averaged over $1000$ evaluation paths and, for turnover and band width, over the $252$ within-path steps (see the metric-definition paragraph, Section~\ref{sec:stage1_results}).}
