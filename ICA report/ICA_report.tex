...
Typical terminal utilities include logarithmic, square-root, and CRRA utility, all of which require an explicit treatment of nonpositive wealth. An earlier, incompletely archived exploratory run did not record the exact utility, utility floor or bankruptcy penalty, or the coefficients $(\lambda_U,\lambda_{to},\lambda_g,\lambda_{gl})$, and is not part of the seed-aggregated inference reported starting in Section~\ref{sec:stage1_results}. The Phase~1A five-seed replication instead uses logarithmic utility with a fixed risk-aversion parameter, a positivity safeguard applied after transaction-cost deduction, and the loss coefficients recorded in Table~\ref{tab:stage2_config} and in the archived manifest for every training seed.
...
Terminal objective & Log utility ($\gamma_{\mathrm{risk}}=5$) plus turnover ($5\times10^{-4}$), gap-$L_2$ ($10^{-5}$), and gross-leverage ($10^{-5}$) regularization, as in \eqref{eq:band_loss} \\
...
\centering
\small
\caption{Common-covariance Stage~1 performance across five independent training seeds. Bracketed values report 95\% Student-$t$ intervals across seed-level statistics (four degrees of freedom). Terminal-wealth dispersion (Std.\ $X_T$) is calculated across evaluation paths within each seed and is distinct from training-seed uncertainty. Equal weight and minimum variance do not depend on the trained policy and are seed-invariant by construction (zero-width interval).}
...
Table~\ref{tab:stage2_means} reports the five-seed transaction-cost-aware comparison. The previously reported legacy single-seed illustration (mean net terminal wealth $1.248$ for the boundary policy against $500$ evaluation paths, versus $1.136$/$1.147$/$1.052$ for the daily-center, monthly-center, and monthly-equal-weight comparators) used an incompletely archived training seed, checkpoint, and evaluation-path set; it is superseded here by the fully traceable Phase~1A five-seed replication (Table~\ref{tab:stage2_config}) and is not mixed into the table below.

\begin{table}[H]
\centering
\small
\caption{Stage~2 transaction-cost-aware performance across five independent training seeds, $1000$ common evaluation paths per seed. Bracketed net-terminal-wealth values report 95\% Student-$t$ intervals across training seeds. Band width is a structural quantity defined only for Center+DNNBand; ``--'' denotes not applicable, not zero.}
...
\section{Complete Phase~1A Paired Comparisons}
