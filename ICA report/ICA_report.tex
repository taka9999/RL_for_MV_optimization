...
\vspace{0.45cm}
{\small\itshape Draft version, July 26, 2026. Repeated-seed robustness analysis is in progress.\par}
\vfill
...
\vspace{0.8em}
\ICAAuthorName\ifx\ICAAffiliation\empty\else, \ICAAffiliation\fi
...
\begin{abstract}
This paper develops a structure-guided learning framework for multi-period portfolio optimization with latent market regimes and proportional transaction costs. Stage~1 studies a precommitted mean--variance problem. Under common observation covariance, Wonham filtering produces a belief-state diffusion, and the entropy-regularized HJB equation yields a Gaussian exploratory policy and a wealth-quadratic value function. These results motivate a low-dimensional actor--critic rather than a generic black-box policy. A statistical jump model supplies regime-dependent drift, covariance, and switching-rate estimates to the online filter. For regime-dependent covariance, an exact Gaussian Bayes correction conditional on a discretized Markov prediction is combined with a moment-matched, belief-averaged control approximation and an approximate HJB structure. Stage~2 uses the frictionless policy as a state-dependent center for costly trading. A transaction-cost QVI provides the theoretical benchmark, but its joint critic/intervention learning was numerically unstable; the reported implementation therefore learns asymmetric no-trade boundaries by direct pathwise policy optimization. Representative simulations show higher mean terminal wealth with comparable or lower dispersion than the displayed static baselines in Stage~1, and higher displayed mean net terminal wealth than frequent center rebalancing in Stage~2. The paper distinguishes theorem-level results, approximate control structure, and numerically usable implementation, and interprets the framework as a portfolio-control building block for long-horizon institutional and actuarial applications rather than a complete asset--liability management model.
\end{abstract}
...
This paper combines two complementary components. Stage~1 learns a frictionless portfolio policy under partial information. It starts from the continuous-time precommitted mean--variance formulation of Wu and Li~\cite{wu2024}, extends its structured actor--critic representation to multiple risky assets, and makes the filtering, policy, value function, and learning pipeline operational. Stage~2 first formulates the transaction-cost problem as a QVI on the enlarged state $(t,x,p,w)$. Because the corresponding critic-based RL implementation was not numerically stable in the current experiments, the reported method uses Direct Boundary RL: a neural policy predicts lower and upper no-trade boundaries around the frozen Stage~1 center and is trained by pathwise differentiation through the wealth recursion with proportional costs. The reported Stage~2 does not claim to solve the exact transaction-cost extension of the Stage~1 mean--variance problem: it uses the Stage~1 policy as a state-dependent structural feature and optimizes a separate transaction-cost-aware terminal objective.
...
\item \textbf{SJM-based regime estimation for regime-switching GBM control.}
We integrate the statistical jump model idea into partially observed multi-asset portfolio optimization under a regime-switching geometric Brownian motion. Persistent offline SJM labels are converted into regime-specific drift, covariance, and transition-intensity estimates, which are then supplied to the online Bayesian filter. The contribution is the integration of an established regime-identification method with the HJB-guided control pipeline, rather than a new statistical theory for SJM.

\item \textbf{An approximate HJB extension to regime-dependent covariance.}
To retain an HJB-guided policy structure when volatility and correlation vary by regime, we combine an exact Gaussian Bayes correction for the specified discretized observation model with a moment-matched, belief-averaged state approximation. We derive the resulting approximate HJB, Gaussian exploratory policy, and coefficient equations, including the nonlinear term that vanishes in the common-covariance benchmark. This is an approximate theoretical contribution; no verification or approximation-error theorem is claimed. In the representative run, the surrogate produced higher mean terminal wealth and lower dispersion than the displayed static baselines, but its noisier training indicates that convergence and approximation quality remain open issues.

\item \textbf{A two-stage learning architecture for portfolio optimization with transaction costs.}
Stage~1 learns a frictionless HJB-guided policy, which is then frozen as a state-dependent center for Stage~2. We formulate the costly problem through a QVI and derive a center-relative critic/intervention parametrization consistent with its continuation and intervention structure. Because joint QVI-residual and intervention learning was numerically unstable, the reported implementation retains the robust structural implications---a frictionless center, an inaction region, feasibility constraints, and pathwise transaction costs---and learns asymmetric boundaries directly. The contribution is a structured two-stage framework that separates frictionless policy learning from transaction-cost correction while optimizing a separate transaction-cost-aware terminal objective in Stage~2.
...
Transaction costs & Classical no-trade regions and HJB-initialized boundary networks & Belief-conditioned two-stage learning with a frozen Stage~1 center and Direct Boundary RL \\
...
\subsection{Legacy threshold labeling}
...
\end{equation}
In the legacy configuration, $Y_l=0.19$ and $Y_u=0.24$ are fixed hyperparameters rather than cross-validated estimates. This method is transparent but sensitive to threshold selection and can produce unreliable labels in a multi-asset setting.

\subsection{Statistical jump model}

The current regime-dependent-covariance experiment instead uses an equal-weight return index and constructs a standardized feature vector $z_k$ from rolling means and volatilities, downside deviation, rolling drawdown, short-versus-long mean spreads, and volatility ratios. Every rolling feature is intended to use information available at or before time $k$ only. A two-state statistical jump model jointly estimates regime representatives $\vartheta_1,\vartheta_2$ and a persistent label path $s_{1:n}$ by solving a problem of the form
\begin{equation}
...
The archived configuration records the estimation horizon and optimization settings, but it does not retain the exact rolling windows, feature-by-feature normalization statistics, or the random seed used to generate the estimation sample. Those items must be recorded in the frozen final manifest. In particular, the final implementation record will specify each lookback window, confirm that standardization statistics are computed from the estimation sample only, and document the separation among SJM fitting, policy training, and held-out simulation evaluation.
...
\end{itemize}
In trial runs, the QVI residuals and intervention trigger did not converge reliably and produced unstable actor updates. The final experiment therefore uses \emph{Direct Boundary RL}: a restricted no-trade policy is optimized directly through simulated trajectories. Here ``RL'' refers to sequential pathwise policy learning from environment interactions; the method does not use a separate transaction-cost critic.

The distinction between the theoretical benchmark and the reported objective is important. The QVI above retains the Stage~1 mean--variance terminal condition, whereas the implemented Direct Boundary RL policy is trained using the terminal utility and regularization objective in \eqref{eq:band_loss}. Stage~2 is therefore not claimed to solve the exact transaction-cost extension of the Stage~1 problem. The frozen Stage~1 policy is used as a state-dependent structural feature and advanced start, and all reported Stage~2 policy comparisons are interpreted under the separate transaction-cost-aware objective.
...
\end{equation}
First apply the componentwise no-trade-band projection
\begin{equation}
\bw_k^{\mathrm{band}}
=\min\bigl(\max(\bw_k^-,\underline{\bw}_k),\overline{\bw}_k\bigr).
\end{equation}
The reported specification also imposes the hard gross-leverage constraint through
\begin{equation}
\bw_k^+
...
\label{eq:projection}
\end{equation}
where $\Pi_{\mathcal W}$ denotes projection onto the admissible leverage set, together with any additional implementation-specific box constraints. Thus no trade occurs inside the band whenever the current position is also feasible; outside the band, the portfolio is moved to the nearest boundary and then made feasible. Unlike a symmetric fixed-width rule, the network permits asymmetric, time-varying, belief-dependent boundaries. The risky-weight vector excludes cash, so the discounted cash weight is $1-\bone^\top\bw_k$.

\subsection{Wealth recursion and direct objective}

To make the trading-time convention explicit, let $(x_k^-,\bw_k^-)$ denote pre-trade discounted wealth and risky weights at time $t_k$. The boundary and feasibility rules in \eqref{eq:projection} produce the post-trade target $\bw_k^+$. With proportional transaction-cost rate $\kappa$, define
\begin{align}
\mathrm{TC}_k
...
The frozen implementation used for the final tables will follow the same pre-/post-trade timing convention. The final manifest will therefore record the recursion version together with the checkpoint and evaluation seeds.
The network is trained by differentiating through \eqref{eq:projection}--\eqref{eq:tc_wealth}. A representative objective is
\begin{equation}
...
&=\E[\|\bw_k^+-\bw_k^-\|_1],\\
\mathcal L_g
&=\E[\|\bg_k^\ell\|_2^2+\|\bg_k^u\|_2^2],\\
\mathcal L_{gl}
&=\E[\|\bw_k^+\|_1].
\end{align}
The physical proportional transaction cost is already deducted from wealth in \eqref{eq:tc_wealth}. The additional turnover term $\mathcal L_{to}$ is therefore a deliberate governance and training regularizer---and a simple proxy for unmodeled market impact---rather than a second accounting deduction of the same transaction cost. The gap and gross-leverage terms discourage uninformatively wide bands and positions close to the admissibility boundary.

Typical terminal utilities include logarithmic, square-root, and CRRA utility, all of which require an explicit treatment of nonpositive wealth. The legacy representative-run artifacts do not identify the exact utility, utility floor or bankruptcy penalty, or the coefficients $(\lambda_U,\lambda_{to},\lambda_g,\lambda_{gl})$. Consequently, that run is retained only as an exploratory numerical illustration and will not enter seed-aggregated inference. The ongoing replication prospectively freezes the utility definition, positivity safeguard after transaction-cost deduction, and all loss coefficients in its manifest.

Direct Boundary RL is related to the HJB advanced-start strategy of Mulvey et al.~\cite{mulvey2020}: a tractable frictionless solution supplies information that makes the transaction-cost problem easier to learn. The QVI derivation above explains why a no-trade region and center-relative coordinates are natural, but the reported optimizer directly trains the restricted boundary policy class because the attempted QVI critic/intervention actor was unstable. The QVI model is retained as the theoretical benchmark and as a target for future stabilization work.
...
\label{alg:unified}
\begin{algorithmic}[1]
\State Construct return-based regime features and fit a two-state statistical jump model.
\State Estimate $\bmu^{(i)}$, $\bSigma^{(i)}$, and $\lambda_i$ from the inferred labels.
...
\For{each Stage~2 training iteration}
  ...
  \State Predict Direct Boundary RL no-trade gaps and apply the projection rule.
...
The transaction-cost experiment uses the common-covariance Stage~1 center, estimated-parameter filtering, proportional transaction cost $\kappa=0.002$, leverage cap $1.5$, and $500$ common evaluation paths. Under \eqref{eq:tc_wealth}, $\kappa=0.002$ represents 20 basis points per unit of absolute pre-to-post-trade risky-weight turnover, scaled by pre-trade wealth. Reported comparators include daily and monthly center rebalancing, equal weight, minimum variance, and the learned boundary policy.
...
\centering
\small
\caption{Selected Stage~1 configuration parameters.}
\label{tab:configs}
\begin{tabular}{lcc}
\toprule
...
Regime estimation & common-covariance fit & statistical jump model \\
Critic steps per outer iteration & $1$ & $1$ \\
Multiplier EMA parameter & $0.5$ & $0.5$ \\
\bottomrule
\end{tabular}
\end{table}
...
\centering
\small
\caption{Stage~2 configuration and reproducibility status for the representative run.}
...
\toprule
Item & Reported setting or current status \\
\midrule
Center policy & Common-covariance Stage~1 policy with estimated-parameter filtering \\
Policy class & Feedforward network with separate softplus lower- and upper-gap heads \\
Terminal objective & Utility term plus turnover, gap-width, and gross-leverage regularization as in \eqref{eq:band_loss} \\
Transaction cost / leverage cap & $\kappa=0.002$ / $1.5$ \\
Evaluation paths & $500$ independently generated common paths \\
Optimizer & Adam \\
Exact utility, positivity safeguard, and $(\lambda_U,\lambda_{to},\lambda_g,\lambda_{gl})$ & Not archived for the legacy representative run; frozen prospectively in the ongoing replication \\
Hidden layers, width, activation, learning rate & Not archived for the legacy representative run; frozen prospectively in the ongoing replication \\
Training iterations and episodes per iteration & Not archived for the legacy representative run; frozen prospectively in the ongoing replication \\
Training seed, center checkpoint, run identifier & Not fully traceable for the legacy run; it is excluded from seed-aggregated inference \\
Evaluation seed set and recursion version & Not fully traceable for the legacy run; frozen prospectively under \eqref{eq:tc_wealth} \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{Draft robustness protocol and numerical provenance}

The tables in this draft report representative single-training-seed results evaluated on independently generated simulation paths. A five-seed replication using a fixed common evaluation set is in progress but was not completed by the draft cutoff. Accordingly, the displayed results should be interpreted as evidence of feasibility and qualitative behavior rather than as statistically significant evidence of general dominance. The final analysis is intended to report seed-aggregated means, standard errors, 95\% confidence intervals, and paired comparisons where applicable.

To avoid mixing results generated under different checkpoints or evaluation protocols, the present tables retain the runs used to generate the adjacent figures. The legacy Stage~2 run is shown only as an exploratory illustration because its complete training manifest and wealth-recursion version were not archived; it is excluded from the seed-aggregated analysis. The ongoing replication uses a frozen specification under which each reported value is linked to a manifest recording the training seed, checkpoint, evaluation-path seed set, number of paths, utility and regularization coefficients, transaction-cost specification, leverage treatment, wealth-recursion version, and gross-versus-net convention. Only results generated under that common specification will be aggregated.

\subsection{Stage 1 results}

Figure~\ref{fig:stage1_common} summarizes the common-covariance benchmark. The learned policy improves during training and produces a higher average terminal outcome than the displayed static benchmarks while maintaining comparable or lower terminal dispersion in this representative run. This case is closest to the theory because both the innovation filter and HJB reduction use common covariance.

\begin{figure}[H]
\centering
\begin{subfigure}[t]{0.32\textwidth}
\centering
\imgorplaceholder{\linewidth}{0.18\textheight}{file/learning_curves_comcov.png}
\caption{Training diagnostics.}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.32\textwidth}
\centering
\imgorplaceholder{\linewidth}{0.18\textheight}{file/avg_wealth_comcov.png}
\caption{Average wealth.}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.32\textwidth}
\centering
\imgorplaceholder{\linewidth}{0.18\textheight}{file/terminal_hist_comcov.png}
\caption{Terminal wealth.}
\end{subfigure}
\caption{Representative Stage~1 results under common covariance.}
...
\centering
\small
\caption{Representative single-seed common-covariance held-out simulation summary.}
\label{tab:common_metrics}
\begin{tabular}{lccccc}
\toprule
Method & Mean $X_T$ & Std. $X_T$ & Var. $X_T$ & $\Pbb(X_T\ge z)$ & Mean shortfall \\
\midrule
Equal weight & $1.070$ & $0.272$ & $0.074$ & $0.309$ & $0.187$ \\
Minimum variance & $1.071$ & $0.261$ & $0.068$ & $0.311$ & $0.182$ \\
HJB-guided RL & $1.197$ & $0.266$ & $0.071$ & $0.611$ & $0.115$ \\
\bottomrule
\end{tabular}
\end{table}

The RL policy raises mean terminal wealth by approximately $0.126$ relative to minimum variance and nearly doubles the target-hit probability, while its standard deviation remains close to the static benchmarks. The improvement is therefore not explained solely by an increase in terminal dispersion.

Figure~\ref{fig:stage1_regcov} reports the regime-dependent covariance extension. Learning is visibly noisier, which is consistent with parameter-estimation error and the additional belief-averaging approximation. Nevertheless, the displayed run continues to show improvement over training and favorable terminal outcomes relative to the static benchmarks. These results should be read as evidence of implementability, not as verification of the approximate control model.

\begin{figure}[H]
\centering
\begin{subfigure}[t]{0.32\textwidth}
\centering
\imgorplaceholder{\linewidth}{0.18\textheight}{file/learning_curve_regcov.png}
\caption{Training diagnostics.}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.32\textwidth}
\centering
\imgorplaceholder{\linewidth}{0.18\textheight}{file/avg_wealth_regcov.png}
\caption{Average wealth.}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.32\textwidth}
\centering
\imgorplaceholder{\linewidth}{0.18\textheight}{file/terminal_hist_regcov.png}
\caption{Terminal wealth.}
\end{subfigure}
\caption{Representative Stage~1 results with regime-dependent covariance.}
\label{fig:stage1_regcov}
\end{figure}

\begin{table}[H]
\centering
\small
\caption{Representative single-seed regime-dependent-covariance held-out simulation summary.}
...
The regime-dependent policy has the highest displayed mean and the lowest dispersion. Its gain in target-hit probability is more modest than in the common-covariance run, and the training curve is noisier. This is consistent with the SJM transition-rate mismatch and with the additional approximation in \eqref{eq:effective_wealth_regcov}--\eqref{eq:regcov_f_pde}. These numbers demonstrate promising behavior for the implemented surrogate; they do not prove optimality for the original regime-dependent continuous-time model.

\subsection{Stage 2 results}

Figure~\ref{fig:stage2_eval} compares the transaction-cost-aware policies. In the displayed experiment, the learned boundary policy has the strongest average net discounted wealth trajectory and a right-shifted terminal wealth distribution relative to center-only and static baselines. Turnover, cumulative transaction costs, and trade counts are not yet reported as separate summary statistics, so the current comparison supports a net-wealth statement rather than a complete performance--turnover decomposition.

\begin{figure}[H]
\centering
\begin{subfigure}[t]{0.49\textwidth}
\centering
\imgorplaceholder{\linewidth}{0.27\textheight}{avg_wealth.png}
\caption{Average discounted wealth paths.}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.49\textwidth}
\centering
\imgorplaceholder{\linewidth}{0.27\textheight}{terminal_hist.png}
\caption{Terminal wealth distributions.}
\end{subfigure}
\caption{Representative Stage~2 evaluation with proportional transaction costs.}
...
\centering
\small
\caption{Legacy representative single-seed illustration of mean terminal discounted wealth net of proportional transaction costs ($500$ independently generated evaluation paths).}
\label{tab:stage2_means}
\begin{tabular}{lc}
\toprule
Policy & Mean net terminal wealth \\
\midrule
Direct Boundary RL around Stage~1 center & $1.248$ \\
Center only, daily rebalance & $1.136$ \\
Center only, monthly rebalance & $1.147$ \\
Equal weight, monthly rebalance & $1.052$ \\
\bottomrule
\end{tabular}
\end{table}

The complete training seed, center checkpoint, evaluation seed set, utility coefficients, and pre-/post-trade recursion version were not archived for this legacy run. Table~\ref{tab:stage2_means} is therefore retained only to document the exploratory result associated with Figure~\ref{fig:stage2_eval}; it will not be included in seed-aggregated inference.

In this test, Direct Boundary RL has mean net terminal wealth $0.101$ above monthly center rebalancing and $0.112$ above daily center rebalancing. The daily center does not dominate the monthly center because more frequent movement toward the frictionless target incurs additional proportional costs. A direct turnover attribution requires the additional turnover and cost statistics planned for the final analysis.

The training diagnostics in Figure~\ref{fig:stage2_training} show improvement in the terminal objective and the direct boundary loss, although the pathwise optimization remains noisy. This behavior is expected because each gradient is computed through simulated returns, belief updates, projection decisions, and accumulated transaction costs. In contrast to the attempted QVI actor--critic, the direct boundary run did not collapse into an always-trade or never-trade policy and produced usable boundaries on independently generated evaluation paths.

\begin{figure}[H]
\centering
\begin{subfigure}[t]{0.49\textwidth}
\centering
\imgorplaceholder{\linewidth}{0.24\textheight}{learning_curve.png}
\caption{Terminal-performance diagnostic.}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.49\textwidth}
\centering
\imgorplaceholder{\linewidth}{0.24\textheight}{loss_curve.png}
\caption{Direct boundary objective.}
\end{subfigure}
\caption{Stage~2 pathwise training diagnostics.}
...
For insurers and pension funds, the target wealth can serve as a reduced-form accumulation or funding objective, while the trading band can reflect an operational preference for stable allocations and controlled turnover. The current results are methodological demonstrations rather than investment recommendations. Their practical value lies in providing a transparent architecture that could later accommodate liability cash flows, capital constraints, prescribed asset limits, and institution-specific transaction budgets.
...
For regime-dependent covariance, the discrete-time Gaussian prediction--correction filter is exact for the specified hidden Markov observation model, but the belief-averaged effective diffusion and its HJB are moment-matched approximations. For transaction costs, the QVI is the theoretical benchmark and the center-relative critic/intervention actor is exploratory. The reported Direct Boundary RL method produced a numerically usable pathwise policy in the displayed experiment, with a separate terminal objective; it is not a numerical solution of that QVI.

\subsection{Lessons from theory-guided learning}

The experiments suggest an asymmetric role for structural theory. In Stage~1, the common-covariance HJB identifies the Gaussian policy family, the linear dependence of its mean on wealth, the quadratic critic, and the lower-dimensional coefficient functions. These restrictions remove large parts of the learning problem without materially narrowing the theorem-level optimizer. The resulting actor--critic provided usable training and favorable displayed results under both common and regime-dependent covariance. Because a controlled structured-versus-unstructured ablation has not yet been completed, the present evidence does not identify the restrictions themselves as the sole cause of the observed performance.

The Stage~2 experience is different. The QVI identifies the enlarged state, the intervention-versus-continuation logic, and the relevance of a no-trade region, but it does not determine the finite-horizon multidimensional boundary geometry as sharply as the Stage~1 HJB determines the Gaussian actor. Jointly learning the QVI critic, obstacle residual, trigger, and intervention map was unstable, while preliminary generic deep-RL and Transformer-style formulations also failed to produce a sufficiently reliable policy within the available experiments. This comparison does not identify the encoder architecture or the absence of a particular parametrization as the unique cause of instability. The Direct Boundary architecture instead imposed economically meaningful center-relative no-trade geometry, retained feasibility constraints and pathwise transaction costs, and produced a numerically usable implementation.
...
The Stage~2 results have a similarly specific interpretation. Daily rebalancing to the frictionless center has lower displayed mean net wealth than monthly rebalancing, consistent with repeated proportional costs offsetting the marginal tracking benefit. Direct Boundary RL has the highest displayed mean net wealth, but the current evidence does not determine whether the difference comes primarily from lower trading frequency, asymmetric boundaries, belief conditioning, or time-to-go conditioning. Those mechanisms require turnover decomposition and ablation analysis.

\subsection{Limitations and robustness status}

The numerical results in this draft are based on representative single-seed runs. A five-seed replication using common independently generated evaluation paths is currently in progress. Until those results are incorporated, the figures and tables should be interpreted as evidence of feasibility and qualitative behavior, not as definitive estimates of performance across training seeds.

Filtering quality depends on the regime segmentation and parameter estimates, and the estimated transition persistence differs materially from the simulator. The regime-dependent covariance control layer is moment-matched rather than exact, and $\widetilde c(p)$ has not been identified from a continuous-time filtering theorem. Stage~2 changes from the Stage~1 mean--variance criterion to a terminal utility objective, so the frictionless center is a structural feature rather than the exact zero-cost optimizer of the Stage~2 objective. The attempted QVI-based RL was unstable, while hard componentwise projection in Direct Boundary RL is interpretable but does not represent the most general multidimensional intervention geometry. Finally, liabilities, solvency capital, taxes, market impact, and governance constraints are not explicitly modeled.
...
\min\{-V_t-\mathcal LV,\ \mathcal MV-V\}=0,
\end{equation}
where $\mathcal M$ accounts for proportional wealth loss and a post-trade target map. The next methodological step is not to formulate this QVI again, but to stabilize its numerical learning: continuation/intervention sampling, obstacle-compatible losses, target networks for $\mathcal M V$, curriculum learning from Direct Boundary RL, and smooth trigger approximations are natural candidates.

Broader sensitivity to transaction-cost rates, entropy weights, and filtering misspecification, together with ablations separating the effects of the belief state, HJB-guided parametrization, martingale signal, full-horizon residual, symmetric versus asymmetric bands, and QVI-derived priors, remain future work unless completed before submission. Stronger benchmarks should include a full-information regime oracle retrained under that information structure, a true-parameter filtered policy, a myopic policy without the belief-hedging term, and a fixed symmetric band. Supplying the true regime directly to a policy trained only on filtered beliefs would instead be an out-of-distribution stress test, not a formal oracle comparison. An actuarial extension should introduce stochastic liability cash flows, funding-ratio or surplus objectives, regulatory capital constraints, and asset eligibility limits so that the portfolio controller becomes a component of a complete ALM problem.

\section{Conclusion}

This paper starts from the view that reinforcement learning is well matched to dynamic portfolio choice when the economically relevant state and admissible policy class are constructed carefully. The latent regime is converted into a filtered belief state, the frictionless actor is restricted by the HJB, and transaction costs are handled through a second-stage no-trade policy around the learned center. The resulting architecture combines state estimation, sequential control, and rebalancing discipline without treating portfolio selection as an unrestricted state-to-weight prediction problem.

The main contributions are the operational multi-asset implementation of partially observed mean--variance actor--critic learning; the integration of SJM-based regime estimation with online filtering; a discretized Gaussian filtering and approximate HJB extension for regime-dependent covariance; and a two-stage transaction-cost framework that connects a QVI benchmark, a formula-based intervention actor, and Direct Boundary learning. The covariance extension is important because changing volatility and correlation directly affect diversification and risk control, even when expected-return differences are difficult to estimate reliably.

The representative numerical results are consistent with the intended role of the structure. In the displayed single-seed runs, the common-covariance HJB-guided policy has more favorable target-wealth outcomes than the static rules, and the regime-dependent covariance surrogate has higher mean terminal wealth and lower dispersion than the displayed static benchmarks. These results suggest that the simple belief-averaged approximation can be useful despite unresolved convergence and verification questions, but they do not establish general or statistically significant dominance. With transaction costs, Direct Boundary RL has higher displayed mean net terminal wealth than daily or monthly center rebalancing. These observations remain conditional on the reported runs and do not yet provide a full turnover or transaction-cost attribution.

The broader methodological conclusion is that the contribution lies not in deep learning alone, but in selectively using financial structure to determine the information state, restrict policy geometry, and design the objective, while retaining flexible learning where the control theory does not identify the optimal functional form sufficiently sharply. Stage~1 uses strong low-dimensional restrictions because the Gaussian actor and quadratic critic are characterized by the HJB. In Stage~2, the free-boundary geometry is less completely identified, so flexible simulation-based boundary learning provided the usable implementation, while the QVI and center-relative formulas remain theoretical and architectural guides. For actuarial and institutional applications, this combination of interpretable belief states, structured policies, and transaction-aware rebalancing offers a tractable portfolio-control building block. A complete ALM implementation would additionally require stochastic liabilities, capital constraints, and institution-specific admissibility rules.
...
+\lambda_{co}\E[(R_{\mathrm{cont}}R_{\mathrm{obs}})^2],
\end{equation}
where $r^-=\min(r,0)$. This was the intended critic-based Stage~2 direction. In practice, simultaneous fitting of these residuals and the intervention actor was unstable, motivating the Direct Boundary RL implementation reported in the main text.
