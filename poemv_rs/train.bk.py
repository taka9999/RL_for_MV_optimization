...
def _backtest_band_on_logret(logret_df: pd.DataFrame,
                            ...
                            gross_lev_max: float | None = None,
                            p0: float = 0.5,
                            band_c: np.ndarray | None = None,
                            gamma_mv: float = 1.0,
                            soft_gate_tau: float | None = None
                            ):
    """
    ...
    if band_c is not None:
        band_c = np.asarray(band_c, float).reshape(d,)
    ...
    for t in range(T):
        ...
        if band_c is not None:
            ...
            delta_t = band_c * (max(float(tcost), 1e-12) ** (1.0 / 3.0)) * a2_scale
        else:
            delta_t = delta
        dev = w - w_star
        ...
        if soft_gate_tau is None or soft_gate_tau <= 0:
            inside = np.all(np.abs(dev) <= delta_t)
            if not inside:
                turn = float(np.sum(np.abs((proj - w) * X[t])))
            ...
            score = np.max(np.abs(dev) / (delta_t + 1e-12)) - 1.0
            g = 1.0 / (1.0 + np.exp(-score / float(soft_gate_tau)))
            w_new = (1.0 - g) * w + g * proj
...
def _inv_softplus(y: float, eps: float = 1e-12) -> float:
    y = float(max(y, eps))
    return float(np.log(np.expm1(y) + eps))

def _sigma_from_belief(p: float, filt_params: FilterParams) -> np.ndarray:
    """
    Regime-belief mixture covariance:
      Sigma(p) = p * Sigma1 + (1-p) * Sigma2
    """
    p = float(np.clip(p, 1e-12, 1.0 - 1e-12))
    S1 = np.asarray(filt_params.Sigma1, dtype=float)
    S2 = np.asarray(filt_params.Sigma2, dtype=float)
    return p * S1 + (1.0 - p) * S2

def _a2_scale_from_wstar_sigma(w_star: np.ndarray,
                               Sigma: np.ndarray,
                               gamma_mv: float = 1.0,
                               eps: float = 1e-12) -> np.ndarray:
    """
    A2 scaling proxy:
      Dii = (Q(w*) Sigma Q(w*))_ii
          = (w_i*)^2 [Sigma_ii - 2 (Sigma w*)_i + w*' Sigma w*]
      Gamma_ii ≈ gamma_mv * Sigma_ii
      scale_i = (Dii / Gamma_ii)^(1/3)

    Returns shape (d,)
    """
    w_star = np.asarray(w_star, dtype=float).reshape(-1)
    Sigma = np.asarray(Sigma, dtype=float)
    Sigma_w = Sigma @ w_star
    quad = float(w_star @ Sigma_w)

    Dii = (w_star ** 2) * (np.diag(Sigma) - 2.0 * Sigma_w + quad)
    Dii = np.maximum(Dii, eps)
...
def _train_band_delta(logret_train_df: pd.DataFrame,
    ...
    theta0 = _inv_softplus(delta_init)
    theta = torch.nn.Parameter(torch.full((d,), float(theta0), dtype=torch.float64, device=device))
    ...
    # IMPORTANT: keep omega FIXED for stable band-width learning.
    omega = torch.tensor(float(0.0 if omega_fixed is None else omega_fixed), dtype=torch.float64, device=device)
    ...
    for it in range(1, iters+1):
        ...
        for b in range(batch_size):
            ...
            #c = torch.nn.functional.softplus(theta)  # (d,) positive
            #delta = c * (kappa13 if cubic_root else 1.0)
            ...
            for t in range(n_steps):
                ...
                c = torch.nn.functional.softplus(theta)  # (d,)
                if cubic_root:
                    Sigma_p = _sigma_from_belief(p, filt_params)
                    a2_scale = _a2_scale_from_wstar_sigma(w_star_np, Sigma_p, gamma_mv=gamma_mv)
                    a2_scale_t = torch.tensor(a2_scale, dtype=torch.float64, device=device)
                    delta_t = c * kappa13 * a2_scale_t
                else:
                    delta_t = c
                dev = w - w_star
                ...
                if soft_gate_tau is None or soft_gate_tau <= 0:
                    inside = torch.all(torch.abs(dev) <= delta_t)
                    if not bool(inside.item()):
                        turn = torch.sum(torch.abs((proj - w) * x))
                        cost = float(tcost) * turn if tcost > 0 else torch.tensor(0.0, dtype=torch.float64, device=device)
                        x_after = x - cost
                        x_after = torch.clamp(x_after, min=1e-12)
                        x = x_after
                        u = proj * x
                else:
                    score = torch.max(torch.abs(dev) / (delta_t + 1e-12)) - 1.0
                    g = torch.sigmoid(score / float(soft_gate_tau))
                    w_new = (1.0 - g) * w + g * proj

                    turn = torch.sum(torch.abs((w_new - w) * x))
                    cost = float(tcost) * turn if tcost > 0 else torch.tensor(0.0, dtype=torch.float64, device=device)
                    x_after = x - cost
                    x_after = torch.clamp(x_after, min=1e-12)
                    x = x_after
            ...
            loss_sum = loss_sum - h
            xT_sum = xT_sum + xT.detach()
        
        loss = loss_sum / float(batch_size)
        xT_mean = xT_sum / float(batch_size)

        opt.zero_grad(set_to_none=True)
        loss.backward()
        opt.step()

        # omega is intentionally fixed here (use center RL's omega).
        ...
        if (it % 50) == 0 or it == 1:
            rows.append({
                ...
                "xT": float(xT_mean.detach().cpu()),
                "omega": float(omega.detach().cpu()),
                #"delta_mean": float(delta.detach().mean().cpu()),
                #"delta": delta.detach().cpu().numpy().tolist(),
                #"c_mean": float(torch.nn.functional.softplus(theta).detach().mean().cpu()),
                #"delta_mean": float((torch.nn.functional.softplus(theta) * (kappa13 if cubic_root else 1.0)).detach().mean().cpu()),
                "delta": (torch.nn.functional.softplus(theta) * (kappa13 if cubic_root else 1.0)).detach().cpu().numpy().tolist(),
                                "c_mean": float(torch.nn.functional.softplus(theta).detach().mean().cpu()),
                "delta_mean_proxy": float((torch.nn.functional.softplus(theta) * (kappa13 if cubic_root else 1.0)).detach().mean().cpu()),
                "c": torch.nn.functional.softplus(theta).detach().cpu().numpy().tolist(),
            })
            pd.DataFrame(rows).to_csv(outdir/"band_train_metrics.csv", index=False)

    #delta_final = (torch.nn.functional.softplus(theta) * (kappa13 if cubic_root else 1.0)).detach().cpu().numpy()
    #np.savez(outdir/"band_delta.npz", delta=delta_final)
    #return delta_final
    c_final = torch.nn.functional.softplus(theta).detach().cpu().numpy()
...
def train(mode: str, iters: int, seed: int, outdir: Path,
          ...
          band_batch_size: int = 8,
          band_cubic_root: bool = True,
          band_fix_omega: bool = True,
    ...
    if eval_logret is not None and (eval_logret.shape[0] > 5):
        # (0) prepare/export center only once if any downstream consumer needs it
        cpath = Path(center_path) if center_path is not None else (outdir/"center_wstar.npz")
        need_center = bool(export_center or band_overlay or band_train)
        p_grid = None
        w_star_grid = None
        if need_center:
            if export_center or (not cpath.exists()):
        ...
        if band_overlay:
            if p_grid is None or w_star_grid is None:
                raise RuntimeError("band_overlay=True requires center grid to be prepared")
            ...
            # learned band: prefer current run's c_final, otherwise fall back to saved artifact
            learned_payload = None
            if c_final is not None:
                learned_payload = {"kind": "c", "value": np.asarray(c_final, dtype=float).reshape(-1,)}
            else:
                ...
                if learned.exists():
                    try:
                        z = np.load(learned)
                        if "c" in z:
                            learned_payload = {"kind": "c", "value": z["c"].astype(float).reshape(-1,)}
                        elif "delta" in z:
            ...
            if learned_payload is not None:
                if learned_payload["kind"] == "c":
                    _, X_band_learned, gross_band_learned = _backtest_band_on_logret(
                        eval_df, filt_params, p_grid, w_star_grid,
                        delta=None, dt=dt, x0=cfg.x0,
                        tcost=float(band_tcost),
                        gross_lev_max=band_gross_lev_max,
                        p0=cfg.p0,
                        band_c=learned_payload["value"],
                        gamma_mv=1.0,
                        soft_gate_tau=band_soft_gate_tau,
                    )
                else:
                    _, X_band_learned, gross_band_learned = _backtest_band_on_logret(
                        eval_df, filt_params, p_grid, w_star_grid,
                        delta=learned_payload["value"], dt=dt, x0=cfg.x0,
                        tcost=float(band_tcost),
                        gross_lev_max=band_gross_lev_max,
                        p0=cfg.p0,
                        soft_gate_tau=band_soft_gate_tau,
...
def main():
    ...
    ap.add_argument("--band_batch_size", type=int, default=8,
                    help="Mini-batch size for band training (episodes per update). Larger => stabler gradients.")
    ap.add_argument("--band_cubic_root", type=int, default=1, choices=[0,1],
                    help="If 1: parameterize delta = c * kappa^(1/3) (c learned). If 0: delta = c.")
    ap.add_argument("--band_fix_omega", type=int, default=1, choices=[0,1],
    ...
    train(args.mode, args.iters, args.seed, Path(args.outdir), args.alpha_theta, args.alpha_phi, args.alpha_w,
              ...
              band_cubic_root=args.band_cubic_root,
              band_fix_omega=args.band_fix_omega,
