...
from .heuristic import HeuristicThresholds, estimate_env_params_from_labeled_returns, label_bull_bear_from_drawdowns
...
def run_filter_demo(outdir: Path, true_params: RSGBMParams, est_params: RSGBMParams, T_years=10.0, dt=1 / 252, seed=0):
    ...
    p_true = np.empty(len(S))
    p_est = np.empty(len(S))
    p_true[0] = 0.5
    p_est[0] = 0.5
...
def _episode_position_stats(traj):
    ...
    x = np.asarray(traj["x"], dtype=float)[:-1]
    denom = np.maximum(np.abs(x), 1e-12)[:, None]
    w = u / denom
    gross_lev = np.sum(np.abs(w), axis=1)
    cash_w = 1.0 - np.sum(w, axis=1)
    abs_u = np.mean(np.abs(u), axis=1)
    return {
...
def train(
    ...
    alpha_w: float,
    Lambda: float,
    omega_update_every: int,
    episode_T_years: float = 1.0,
    dt: float = 1 / 252,
    ...
    cfg = TrainConfig(
        T_years=episode_T_years, dt=dt, x0=1.0, s0=1.0, p0=0.5,
        a_max=a_max, cap_mode=cap_mode, z=z, Lambda=Lambda, r=r,
        alpha_theta=alpha_theta, alpha_phi=alpha_phi, alpha_w=alpha_w,
        omega_update_every=omega_update_every, critic_steps=critic_steps,
        advantage_norm_eps=advantage_norm_eps, omega_ema_beta=omega_ema_beta,
        mu1=policy_params.mu1, mu2=policy_params.mu2,
    ...
    for m in range(1, iters + 1):
        ...
        batch_xT = []
        batch_stats = []
        for j in range(int(episodes_per_iter)):
            env = RSGBMEnv(true_params, EpisodeConfig(
                T_years=episode_T_years, dt=dt, x0=cfg.x0, s0=np.array([cfg.s0, cfg.s0]),
                p0=cfg.p0, a_max=(a_max if a_max is not None else 0.0),
                omega=float(agent.omega.detach().cpu()), seed=seed + 100000 * m + j,
            ))
            obs = env.reset()
            n = env.n_steps
            t_arr = np.empty(n + 1)
            ...
            steps_done = k + 1
            traj = dict(
                t=t_arr[:steps_done + 1], x=x_arr[:steps_done + 1], p=p_arr[:steps_done + 1],
                u=u_arr[:steps_done], u_raw=u_raw_arr[:steps_done], a_max=a_max, cap_mode=cap_mode,
            )
            batch_trajs.append(traj)
            batch_xT.append(float(traj["x"][-1]))
            batch_stats.append(_episode_position_stats(traj))

        loss_c, loss_a = agent.update_from_episodes(batch_trajs)
        xT_mean = float(np.mean(batch_xT))
        last_batch_means.append(xT_mean)
        if len(last_batch_means) > omega_update_every:
            last_batch_means = last_batch_means[-omega_update_every:]

        last_domega = 0.0
        if (m % omega_update_every) == 0:
            mean_xT = float(np.mean(last_batch_means))
        ...
        pos_stats = {k: float(np.mean([s[k] for s in batch_stats])) for k in batch_stats[0]}
        rows.append(dict(
            iter=m,
            omega=float(agent.omega.detach().cpu()),
            xT=xT_mean,
            mean_terminal=float(np.mean(last_batch_means)) if last_batch_means else xT_mean,
            target_z=cfg.z,
            recent_mean_xT=last_mean_xT_window,
            recent_gap_to_z=last_gap,
            delta_omega=last_domega,
            loss_critic=loss_c,
            loss_actor=loss_a,
    ...
    plt.ylabel("mean terminal wealth (recent batch window)")
    fig.tight_layout()
    ...
    with open(outdir / "run_config.json", "w", encoding="utf-8") as f:
        json.dump(dict(
            mode=mode, seed=seed, iters=iters, episodes_per_iter=episodes_per_iter,
            T=episode_T_years, dt=dt, a_max=a_max, cap_mode=cap_mode, r=r,
            x0=cfg.x0, p0=cfg.p0, z=cfg.z, Lambda=Lambda,
            alpha_theta=alpha_theta, alpha_phi=alpha_phi, alpha_w=alpha_w,
            omega_update_every=omega_update_every, critic_steps=critic_steps,
            advantage_norm_eps=advantage_norm_eps, omega_ema_beta=omega_ema_beta,
            true_params=true_params.__dict__, filter_params=filt_params.__dict__,
            estimated_params=est_params.__dict__, policy_params=policy_params.__dict__,
...
def main():
    ...
    ap.add_argument("--a_max", type=float, default=None)
    ...
    ap.add_argument("--cap_mode", type=str, default="none", choices=["none", "component_tanh", "l1_radial"])
    args = ap.parse_args()
    train(
        args.mode, args.iters, args.seed, Path(args.outdir), args.alpha_theta, args.alpha_phi, args.alpha_w,
        args.Lambda, args.omega_update_every, episode_T_years=args.T, dt=args.dt, a_max=args.a_max,
        r=args.r, critic_steps=args.critic_steps, advantage_norm_eps=args.advantage_norm_eps,
        omega_ema_beta=args.omega_ema_beta, episodes_per_iter=args.episodes_per_iter,
        z=args.z, cap_mode=args.cap_mode,
    )
