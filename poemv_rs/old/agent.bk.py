...
class TrainConfig:
    ...
    cap_mode: str = "none"  # none, component_tanh, l1_radial
    z: float = 1.2
    Lambda: float = 1.0
    r: float = 0.01
...
def _squash_action_np(z: np.ndarray, x: float, a_max: float | None, cap_mode: str) -> np.ndarray:
    ...
    if cap_mode == "l1_radial":
        ...
        if l1 <= 1e-12:
            return z.copy()
        radius = scale * np.tanh(l1 / max(scale, 1e-12))
        return (radius / l1) * z
...
def _squash_log_prob(
    ...
    if cap_mode == "component_tanh":
        ...
        return dist.log_prob(z_raw) - log_det, dist.entropy() + log_det.detach()
    if cap_mode == "l1_radial":
        # proxy: keep raw Gaussian log-prob / entropy. Action map is smooth and bounded,
        # but full Jacobian for L1-radial squash is cumbersome.
        return dist.log_prob(z_raw), dist.entropy()
    raise ValueError(f"Unknown cap_mode={cap_mode}")
...
class POEMVAgent:
    def __init__(self, cfg: TrainConfig):
        ...
        self.mean_xT_ema: float | None = None
    ...
    def policy_mean(self, t: float, x: float, p: float) -> np.ndarray:
        ...
        with torch.no_grad():
            z = self._policy_dist(t_t, x_t, p_t).mean.squeeze(0).detach().cpu().numpy()
    ...
    def _episode_losses(self, traj: dict) -> tuple[torch.Tensor, torch.Tensor]:
        cfg = self.cfg
        ...
        dt_t = torch.as_tensor(cfg.dt, dtype=cfg.dtype, device=cfg.device)
        t0, x0, p0 = t[:-1], x[:-1], p[:-1]

        with torch.no_grad():
            dist_b = self._policy_dist(t0, x0, p0)
            _, entropy_b = _squash_log_prob(dist_b, z_raw, x0, cfg.a_max, cfg.cap_mode)
            entropy_tail = torch.flip(torch.cumsum(torch.flip(entropy_b * dt_t, dims=[0]), dim=0), dims=[0])
        ...
        logp_actor, _ = _squash_log_prob(dist_actor, z_raw, x0, cfg.a_max, cfg.cap_mode)
        loss_a = -(logp_actor * A.detach()).mean()
        return loss_c, loss_a

    def update_from_episode(self, traj: dict) -> tuple[float, float]:
        return self.update_from_episodes([traj])

    def update_from_episodes(self, trajs: list[dict]) -> tuple[float, float]:
        if not trajs:
        ...
        self.opt_phi.step()
        return float(loss_critic), float(loss_a.detach().cpu())
    ...
    def policy_values(self):
        p1 = self.pi.phi1.detach().cpu().numpy()
        p2 = self.pi.phi2.detach().cpu().numpy()
        return {
            "phi1_asset1": float(p1[0]),
            "phi2_asset1": float(p2[0]),
            "phi1_asset2": float(p1[1]),
            "phi2_asset2": float(p2[1]),
        }
